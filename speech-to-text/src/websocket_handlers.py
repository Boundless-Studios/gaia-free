...
from .services.voice_detection import get_voice_detection_service
...
async def handle_transcription_websocket(websocket: WebSocket, user_info: Optional[Dict[str, Any]] = None):
    """
    Handle continuous audio transcription via WebSocket with authentication

    Args:
        websocket: FastAPI WebSocket connection
        user_info: Authenticated user information from Auth0
    """
    await _accept_websocket_with_subprotocol(websocket)
    
    session_id = None
    stt_service = get_elevenlabs_stt_service()
    voice_detector = get_voice_detection_service()
    audio_recorder = get_audio_recorder()
    
    # Use authenticated user info if provided
    authenticated = user_info is not None
    user_id = user_info.get("user_id", "websocket_user") if user_info else "websocket_user"
    ...
    # Session metadata for conversational mode and user tracking
    session_metadata = {
    ...
    try:
        # Wait for first message to start the session
        first_message = await websocket.receive()
        if "text" in first_message:
            text_data = first_message["text"]
            json_data = json.loads(text_data)

            # Extract conversational mode and user metadata
            if "metadata" in json_data:
                metadata = json_data["metadata"]
                session_metadata["conversational_mode"] = metadata.get("conversational_mode", False)
                session_metadata["user_email"] = metadata.get("user_email")
                session_metadata["character_id"] = metadata.get("character_id")
                logger.info(f"Session metadata: conversational_mode={session_metadata['conversational_mode']}, user_email={session_metadata['user_email']}")

        # Create session with authenticated user_id
        session_id = audio_recorder.create_session(user_id=user_id)
        ...
        await websocket.send_json({
            ...
            "data": {
                ...
                "timestamp": time.time(),
                "conversational_mode": session_metadata["conversational_mode"]
            }
        })
        
        # Enhanced buffering system (exact match to backend)
        from collections import deque
        
        # Buffer audio chunks using centralized config
        audio_buffer = deque(maxlen=settings.max_buffer_chunks)  # Circular buffer for all audio
        
        # Voice detection state
        voice_chunks = []  # Chunks to be sent for transcription
        voice_started = False
        last_voice_time = 0
            ...
            try:
                ...
                if "bytes" in message:
                    ...
                    if json_data.get("type") == "stop":
                        ...
                        continue
                    elif json_data.get("event") == "start_continuous":
                        # Skip if already handled during auth
                        logger.debug(f"Start continuous event received again for session {session_id}")
                        continue
                    elif json_data.get("event") == "audio_chunk_realtime":
                        # Skip audio_chunk_realtime events - they're not used for processing
                        continue
                    elif json_data.get("event") == "audio_segment":
                        # Handle audio segment from frontend - this is the actual audio to process
                        segment_data = json_data.get("data", {})
                        audio_b64 = segment_data.get("audio")
                        if audio_b64:
                            data = base64.b64decode(audio_b64)
                            timestamp = segment_data.get("timestamp", time.time() * 1000)
                    elif json_data.get("event") == "pause_transcription":
                        logger.info(f"Pausing transcription for session {session_id}")
                        
                        # If we have accumulated voice chunks, transcribe them first
                        if voice_started and voice_chunks:
                            try:
                                combined_audio = b''.join(voice_chunks)
                                total_size = len(combined_audio)
                                logger.info(f"Transcribing {len(voice_chunks)} chunks ({total_size} bytes) before pause")
                                ...
                                if result.get("text"):
                                    # Store transcription
                                    audio_recorder.add_transcription(session_id, result["text"])
                                    
                                    logger.info(f"Pre-pause transcription: {result['text']}")
                                    ...
                                    await websocket.send_json({
                                        ...
                                        "data": {
                                            ...
                                            "is_partial": True,  # Indicate this is a partial transcription
                                            "timestamp": time.time()
                                        }
                                    })
                            except Exception as e:
                                logger.error(f"Error transcribing before pause: {e}")
                        
                        # Reset voice detection state when paused
                        voice_started = False
                        voice_chunks = []
                        last_voice_time = 0
                        # Clear the voice activity tracker for this session
                        if session_id in voice_activity_tracker:
                            del voice_activity_tracker[session_id]
                        
                        await websocket.send_json({
                            "event": "transcription_paused",
                            "data": {"session_id": session_id}
                        })
                        continue
                    elif json_data.get("event") == "resume_transcription":
                        # Reset voice detection state when resumed
                        logger.info(f"Resuming transcription for session {session_id}")
                        voice_started = False
                        voice_chunks = []
                        last_voice_time = 0
                        await websocket.send_json({
                            "event": "transcription_resumed",
                            "data": {"session_id": session_id}
                        })
                        continue
                    elif json_data.get("event") == "stop_continuous":
                        # Handle stop request
                        logger.info(f"Stopping continuous transcription for session {session_id}")
                        
                        # Transcribe any remaining voice chunks before stopping
                        if voice_started and voice_chunks:
                            try:
                                combined_audio = b''.join(voice_chunks)
                                total_size = len(combined_audio)
                                logger.info(f"Final transcription of {len(voice_chunks)} chunks ({total_size} bytes) before stop")
                    ...
                    # Handle base64 audio data (legacy format)
                    elif "audio" in json_data:
                        data = base64.b64decode(json_data["audio"])
                        logger.debug(f"Decoded legacy audio format: {len(data)} bytes")
                
                if data:
                    # Add to recorder
                    audio_recorder.add_audio_chunk(session_id, data)
                    audio_size = len(data)
                    
                    # Use timestamp from data or current time
                    timestamp = time.time() * 1000  # milliseconds
                    if 'timestamp' in locals() and timestamp:
                        pass  # Use the timestamp from audio_segment
                    else:
                        timestamp = time.time() * 1000
                    
                    # Add to circular buffer (exact match to backend)
                    chunk_data = {
                        'audio': data,
                        'timestamp': timestamp,
                        'size': audio_size
                    ...
                    # Detect voice activity using frequency-based detection (exact match to backend)
                    has_voice, confidence = voice_detector.detect_voice_in_webm(data, use_advanced=True)
                    
                    if has_voice:
                        voice_status = "🎤 VOICE DETECTED" if has_voice else "🔇 SILENCE"
                        
                        # Log detection results (exactly like backend)
                        logger.info(f"Chunk at {timestamp}: {audio_size} bytes - {voice_status} (confidence: {confidence:.1f}%)")
                    
                    # Update voice activity tracker for the indicator (exactly like backend)
                    if has_voice:
                        current_time = time.time() * 1000
                        voice_activity_tracker[session_id] = current_time
                    ...
                    # Voice state machine (exact match to backend)
                    if not voice_started and has_voice:
                        # Voice just started
                        logger.info("🎤 Voice started - beginning collection")
                        voice_started = True
                        voice_chunks = []
                        
                        # Add the previous chunk if available (exact match to backend)
                        if len(audio_buffer) >= 2:
                            prev_chunk = audio_buffer[-2]
                            voice_chunks.append(prev_chunk['audio'])
                            logger.info(f"Added previous chunk ({prev_chunk['size']} bytes) before voice")
                        
                        # Add current chunk
                        voice_chunks.append(data)
                        
                    elif voice_started and has_voice:
                        # Voice continuing
                        voice_chunks.append(data)
                        
                    elif voice_started and not has_voice:
                        # Voice stopped, check if silence threshold reached
                        silence_duration = timestamp - last_voice_time
                        
                        if silence_duration >= silence_threshold_ms:
                            # Silence threshold reached, transcribe accumulated audio
                            logger.info(f"🔇 Silence detected after {silence_duration}ms - transcribing {len(voice_chunks)} chunks")
                            
                            if voice_chunks:
                                try:
                                    # Combine all voice chunks
                                    combined_audio = b''.join(voice_chunks)
                                    total_size = len(combined_audio)
                                    logger.info(f"Sending {total_size} bytes to ElevenLabs for transcription")
                                    ...
                                    if result.get("text"):
                                        ...
                                        logger.info(f"Transcription for session {session_id}: {result['text']}")
                                        ...
                                        # In conversational mode, also send conversation_pause_detected
                                        if session_metadata.get("conversational_mode"):
                                            logger.info(f"🗣️ Conversational mode: Auto-detected pause, sending conversation event")
                                            await websocket.send_json({
                                                "event": "conversation_pause_detected",
                                                "data": {
                                                    "text": result["text"],
                                                    "session_id": session_id,
                                                    "user_email": session_metadata.get("user_email"),
                                                    "character_id": session_metadata.get("character_id"),
                                                    "confidence": result.get("confidence", 1.0),
                                                    "timestamp": time.time()
                                                }
                                            })
                            ...
                            # Still within silence threshold, add chunk (exact match to backend)
                            voice_chunks.append(data)
        ...
        # Clean up
        if session_id:
            # Final transcription if voice chunks have data
            if 'voice_chunks' in locals() and voice_chunks:
                try:
                    # Combine remaining voice chunks
                    combined_audio = b''.join(voice_chunks)
                    logger.info(f"Final transcription of {len(combined_audio)} bytes for session {session_id}")
                    ...
                    logger.error(f"Error in final transcription for session {session_id}: {e}")
            
            # Stop and save session
            await audio_recorder.stop_session(session_id)
            voice_detector.clear_session(session_id)
            
            # Clear voice activity tracker (exactly like backend)
            if session_id in voice_activity_tracker:
                del voice_activity_tracker[session_id]
