...
class Auth0JWTVerifier:
    """Handles Auth0 JWT token verification"""
    ...
    def __init__(self):
        ...
        logger.debug(f"Auth0 JWT verifier initialized with domain: {self.auth0_domain}")
    ...
    def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            user_info = {
                ...
                "roles": payload.get("https://gaia.api/roles", []),  # Custom namespace for roles
                "is_admin": "admin" in payload.get("https://gaia.api/roles", []),
                # Token metadata
                "exp": payload.get("exp"),
                "iat": payload.get("iat"),
            ...
            logger.debug(f"[AUTH0_DEBUG] Extracted user_info:")
            logger.debug(f"[AUTH0_DEBUG]   - user_id: {user_info['user_id']}")
            logger.debug(f"[AUTH0_DEBUG]   - email: {user_info['email']}")
            logger.debug(f"[AUTH0_DEBUG]   - email_verified: {user_info['email_verified']}")
    ...
    def verify_access_token(self, token: str) -> Optional[Dict[str, Any]]:
        """Verify an Auth0 access token - alias for verify_token for compatibility"""
...
def initialize_auth0_verifier():
    ...
    if os.getenv("AUTH0_DOMAIN"):
        try:
            ...
            logger.info("Auth0 JWT verifier initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize Auth0 JWT verifier: {e}")
            return False
    else:
        logger.info("Auth0 not configured, skipping JWT verifier initialization")
        return False
