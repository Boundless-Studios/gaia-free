...
- [Character Presence Pipeline](#character-presence-pipeline)
...
- **Communication**: RESTful APIs with OpenAPI/JSON for structured data
...
- **`main.py`** - FastAPI application entry point
  ...
  - OpenAPI v2 endpoints (`/api/chat`, `/api/campaigns/*`)
...
- **`response_builder.py`** - Response construction utilities
  - Builds OpenAPI-compliant responses
  - Handles error responses
  - Message formatting with Pydantic models
...
## Character Presence Pipeline

Gaia separates “who is here?” decisions between probabilistic agents and deterministic services:

- **Agents propose intent.** Scene-oriented agents (e.g. `CharacterExtractorAgent`, `SceneCreator`) emit names, rough roles, and narrative notes; they never assign canonical IDs or persist state.
- **SceneIntegration curates context.** It holds the cached scene summaries, detects transitions, and asks the roster manager to bootstrap or update presence without owning identifier logic.
- **SceneRosterManager canonicalizes state.** It consumes agent hints, resolves characters against the campaign `CharacterManager`, generates `npc:` identifiers, and records authoritative presence lists (`npcs_present`, `npcs_involved`, `npc_display_names`).
- **Character helpers live under `core/character`.** `CharacterManager` continues to resolve and persist entities, while `CharacterPresenceFormatter` composes roster metadata into friendly, hostility-aware outputs for orchestration layers like `SmartAgentRouter`.
- **Extraction toolkit replaces regex detectors.** The `core/character/extraction` package (descriptors, results, service) is the supported path for interpreting agent output; older regex-based detectors have been retired.
- **Shared conventions.** Orchestrators rely on the formatter output and avoid duplicating string heuristics, while agents keep producing high-level semantic context. This boundary keeps AI output advisory and leaves invariants—IDs, deduplication, hostility flags—to deterministic code paths.
...
- **`simple_campaign_manager.py`** - Campaign lifecycle management

**Character Management** (`src/core/character/`):
- **`character_manager.py`** - Campaign-aware character persistence and lookup
- **`extraction/`** - Shared descriptors, results, and extraction service used by agent tooling
- **`presence_formatter.py`** - Converts roster data into friendly/hostility-aware presence snapshots for orchestrators
...
├── POST /api/campaigns/new     # Create new campaign
├── POST /api/campaigns/blank   # Create blank campaign
└── POST /api/context           # Add context to campaign
```

### OpenAPI Schema

The communication protocol uses Pydantic models for type safety:

```python
# Core message types
- ChatRequest         # Player messages to the system
- MachineResponse     # AI agent responses
- SystemEvent         # System notifications
- ErrorMessage        # Error handling

# Structured game data
- StructuredGameData  # Rich D&D session data
- ConversationHistory # Session history
- ExecutionDetails    # Performance metrics
```
...
4. **JSON Serialization** - Convert to OpenAPI/JSON format
...
3. **Flexibility**: Standard OpenAPI/JSON communication with automatic documentation
